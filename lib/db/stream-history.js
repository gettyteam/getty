  ...
  return await withClient(async (client) => {
    const sql = `
      SELECT 
        ...
        (SELECT COUNT(DISTINCT payload->>'author') FROM stream_samples WHERE tenant_id = s.tenant_id AND sample_at >= s.segment_start AND sample_at <= COALESCE(s.segment_end, NOW()) AND payload->>'author' IS NOT NULL) as unique_chatters
