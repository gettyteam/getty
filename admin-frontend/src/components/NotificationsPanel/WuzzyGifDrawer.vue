<template>
  <transition name="gif-library-fade">
    <div
      ...
      role="dialog"
      aria-modal="true"
      :aria-label="t('wuzzyDrawerTitle')"
      @click.self="emitClose">
      <div class="gif-library-panel wuzzy-drawer-panel">
        <header class="gif-library-header">
          <div class="gif-library-header-text">
            <h2 class="gif-library-title">{{ t('wuzzyDrawerTitle') }}</h2>
            <p class="gif-library-subtitle">{{ t('wuzzyDrawerSubtitle') }}</p>
          </div>
        ...
        <section class="wuzzy-drawer-body" aria-live="polite">
          <WuzzyGifSearch @select="handleSelect" />
        </section>
      </div>
    </div>
  </transition>
...
import WuzzyGifSearch from './WuzzyGifSearch.vue';
