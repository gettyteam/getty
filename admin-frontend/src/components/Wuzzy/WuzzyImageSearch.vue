<template>
  <div class="wuzzy-panel">
    <div class="wuzzy-header">
      <div>
        <h4 class="wuzzy-title">{{ t('wuzzyImageSearchTitle') }}</h4>
        <p class="wuzzy-hint">{{ t('wuzzyImageProviderDesc') }}</p>
      </div>
...
import { searchWuzzyImages, formatBytes } from '../../services/wuzzySearch';
...
async function executeSearch(reset = false) {
  ...
  try {
    ...
    const response = await searchWuzzyImages({
      query,
