  ...
  <Teleport to="body">
    <transition name="gif-library-fade">
      <div
        ...
        role="dialog"
        aria-modal="true"
        :aria-label="t('wuzzyAudioDrawerTitle')"
        @click.self="emitClose">
        <div class="gif-library-panel wuzzy-drawer-panel">
          <header class="gif-library-header">
            <div class="gif-library-header-text">
              <h2 class="gif-library-title">{{ t('wuzzyAudioDrawerTitle') }}</h2>
              <p class="gif-library-subtitle">{{ t('wuzzyAudioDrawerSubtitle') }}</p>
            </div>
          ...
          <section class="wuzzy-drawer-body" aria-live="polite">
            <WuzzyAudioSearch @select="handleSelect" />
          </section>
        </div>
      </div>
    </transition>
...
import WuzzyAudioSearch from './WuzzyAudioSearch.vue';
